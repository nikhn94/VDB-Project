...
FL_W = data{4}.data(:,1)*0.32;
FR_W = data{4}.data(:,2)*0.32;
RL_W = data{4}.data(:,3)*0.32;
RR_W = data{4}.data(:,4)*0.32;
% PID_1mu
FL_PID = data{7}.data(:,1)*0.32;
FR_PID = data{7}.data(:,2)*0.32;
RL_PID = data{7}.data(:,3)*0.32;
RR_PID = data{7}.data(:,4)*0.32;

figure(37);

t  = data{4}.data(:,6);
plot(t, FL_W, '-','Color', [.1 .4 .9]);
hold on;
plot(t, FR_W, '--','Color', [.1 .4 .9]);
plot(t, RL_W, '-.','Color', [.1 .4 .9]);
plot(t, RR_W, ':','Color', [.1 .4 .9]);
t  = data{7}.data(:,6);
plot(t, FL_PID, '-','Color', [.9 .3 .2]);
plot(t, FR_PID, '--','Color', [.9 .3 .2]);
plot(t, RL_PID, '-.','Color', [.9 .3 .2]);
plot(t, RR_PID, ':','Color', [.9 .3 .2]);
xlabel('Time [s]');
ylabel('Wheel speed [m/s]');
legend('FL (LQR)','FR (LQR)','RL (LQR)','RR (LQR)', 'FL (PID)','FR (PID)','RL (PID)','RR (PID)');
hold off;
grid on;
set(gca, 'YColor', 'k');
axis([10 18 -5 40]);
